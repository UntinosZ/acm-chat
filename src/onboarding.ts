...
const channel = "acm-chat" as const;
...
function setGoogleChatDmPolicy(cfg: OpenClawConfig, policy: DmPolicy) {
  const allowFrom =
    policy === "open"
      ? addWildcardAllowFrom(cfg.channels?.["acm-chat"]?.dm?.allowFrom)
      : undefined;
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
      "acm-chat": {
        ...cfg.channels?.["acm-chat"],
        dm: {
          ...cfg.channels?.["acm-chat"]?.dm,
          policy,
          ...(allowFrom ? { allowFrom } : {}),
        },
      },
...
async function promptAllowFrom(params: {
  ...
  const current = params.cfg.channels?.["acm-chat"]?.dm?.allowFrom ?? [];
  ...
  return {
    ...
    channels: {
      ...params.cfg.channels,
      "acm-chat": {
        ...params.cfg.channels?.["acm-chat"],
        enabled: true,
        dm: {
          ...params.cfg.channels?.["acm-chat"]?.dm,
          policy: "allowlist",
          allowFrom: unique,
        },
      },
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  label: "ACM Chat",
  channel,
  policyKey: "channels.acm-chat.dm.policy",
  allowFromKey: "channels.acm-chat.dm.allowFrom",
  getCurrent: (cfg) => cfg.channels?.["acm-chat"]?.dm?.policy ?? "pairing",
  ...
  return {
    ...
    channels: {
      ...
      "acm-chat": {
        ...
        enabled: true,
        accounts: {
          ...cfg.channels?.["acm-chat"]?.accounts,
          [accountId]: {
            ...cfg.channels?.["acm-chat"]?.accounts?.[accountId],
            enabled: true,
            ...patch,
          },
        },
...
async function promptAudience(params: {
  ...
  const audience = await params.prompter.text({
    ...
    placeholder: audienceType === "project-number" ? "1234567890" : "https://your.host/acm-chat",
...
async function noteGoogleChatSetup(prompter: WizardPrompter) {
  await prompter.note(
    [
      ...
      `Docs: ${formatDocsLink("/channels/acm-chat", "channels/acm-chat")}`,
...
export const googlechatOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    const override = accountOverrides["acm-chat"]?.trim();
