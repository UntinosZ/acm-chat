...
export const googlechatDock: ChannelDock = {
  ...
  threading: {
    resolveReplyToMode: ({ cfg }) => cfg.channels?.["acm-chat"]?.replyToMode ?? "off",
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  reload: { configPrefixes: ["channels.acm-chat"] },
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(cfg.channels?.["acm-chat"]?.accounts?.[resolvedAccountId]);
      const allowFromPath = useAccountPath
        ? `channels.acm-chat.accounts.${resolvedAccountId}.dm.`
        : "channels.acm-chat.dm.";
      return {
        policy: account.config.dm?.policy ?? "pairing",
        allowFrom: account.config.dm?.allowFrom ?? [],
        allowFromPath,
        approveHint: formatPairingApproveHint("acm-chat"),
    ...
    collectWarnings: ({ account, cfg }) => {
      ...
      const { groupPolicy } = resolveAllowlistProviderRuntimeGroupPolicy({
        providerConfigPresent: cfg.channels?.["acm-chat"] !== undefined,
        groupPolicy: account.config.groupPolicy,
        defaultGroupPolicy,
      });
      if (groupPolicy === "open") {
        warnings.push(
          `- ACM Chat spaces: groupPolicy="open" allows any space to trigger (mention-gated). Set channels.acm-chat.groupPolicy="allowlist" and configure channels.acm-chat.groups.`,
        );
      }
      if (account.config.dm?.policy === "open") {
        warnings.push(
          `- ACM Chat DMs are open to anyone. Set channels.acm-chat.dm.policy="pairing" or "allowlist".`,
        );
      }
      return warnings;
    },
      ...
      return {
        ...
        channels: {
          ...
          "acm-chat": {
            ...
            enabled: true,
            accounts: {
              ...next.channels?.["acm-chat"]?.accounts,
              [accountId]: {
                ...next.channels?.["acm-chat"]?.accounts?.[accountId],
                enabled: true,
                ...configPatch,
              },
            },
    ...
    resolveTarget: ({ to }) => {
      ...
      if (trimmed) {
        ...
        if (!normalized) {
          return {
            ok: false,
            error: missingTargetError("ACM Chat", "<spaces/{space}|users/{user}>"),
          };
        }
    ...
    sendMedia: async ({ cfg, to, text, mediaUrl, accountId, replyToId, threadId }) => {
      ...
      const maxBytes = resolveChannelMediaMaxBytes({
        ...
        resolveChannelLimitMb: ({ cfg, accountId }) =>
          (
            cfg.channels?.["acm-chat"] as
              | { accounts?: Record<string, { mediaMaxMb?: number }>; mediaMaxMb?: number }
              | undefined
          )?.accounts?.[accountId]?.mediaMaxMb ??
          (cfg.channels?.["acm-chat"] as { mediaMaxMb?: number } | undefined)?.mediaMaxMb,
        accountId,
      });
      ...
      accounts.flatMap((entry) => {
        ...
        if (!entry.audience) {
          issues.push({
            ...
            accountId,
            kind: "config",
            message: "ACM Chat audience is missing (set channels.acm-chat.audience).",
        ...
        if (!entry.audienceType) {
          issues.push({
            ...
            accountId,
            kind: "config",
            message: "ACM Chat audienceType is missing (app-url or project-number).",
            fix: "Set channels.acm-chat.audienceType and channels.acm-chat.audience.",
          });
        }
        return issues;
