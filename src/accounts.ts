...
function listConfiguredAccountIds(cfg: OpenClawConfig): string[] {
  const accounts = cfg.channels?.["acm-chat"]?.accounts;
...
export function resolveDefaultGoogleChatAccountId(cfg: OpenClawConfig): string {
  const channel = cfg.channels?.["acm-chat"];
...
function mergeGoogleChatAccountConfig(
  ...
  const raw = cfg.channels?.["acm-chat"] ?? {};
...
function resolveCredentialsFromConfig(params: {
  ...
  if (isSecretRef(account.serviceAccount)) {
    throw new Error(
      `channels.acm-chat.accounts.${accountId}.serviceAccount: unresolved SecretRef "${account.serviceAccount.source}:${account.serviceAccount.provider}:${account.serviceAccount.id}". Resolve this command against an active gateway runtime snapshot before reading it.`,
    );
  }

  if (isSecretRef(account.serviceAccountRef)) {
    throw new Error(
      `channels.acm-chat.accounts.${accountId}.serviceAccount: unresolved SecretRef "${account.serviceAccountRef.source}:${account.serviceAccountRef.provider}:${account.serviceAccountRef.id}". Resolve this command against an active gateway runtime snapshot before reading it.`,
    );
  }
...
export function resolveGoogleChatAccount(params: {
  ...
  const baseEnabled = params.cfg.channels?.["acm-chat"]?.enabled !== false;
