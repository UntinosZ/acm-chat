...
export function isSenderAllowed(
  ...
  return allowFrom.some((entry) => {
    ...
    // Accept `acm-chat:<id>` but treat `users/...` as an *ID* only (deprecated `users/<email>`).
    const withoutPrefix = normalized.replace(/^(acm-chat|acmchat):/i, "");
...
> {
  ...
  const { groupPolicy, providerMissingFallbackApplied } =
    resolveAllowlistProviderRuntimeGroupPolicy({
      providerConfigPresent: config.channels?.["acm-chat"] !== undefined,
      groupPolicy: account.config.groupPolicy,
      defaultGroupPolicy,
    });
  warnMissingProviderGroupPolicyFallbackOnce({
    providerMissingFallbackApplied,
    providerKey: "acm-chat",
    accountId: account.accountId,
  ...
  if (!isGroup) {
    ...
    if (access.decision !== "allow") {
      if (access.decision === "pairing") {
        ...
        if (created) {
          logVerbose(`acm-chat pairing request sender=${senderId}`);
          try {
  ...
  if (
    ...
    logVerbose(`acm-chat: drop control command from ${senderId}`);
    return { ok: false };
  }
